...
namespace DungeonTeam.Gameplay.Skills.Runtime
{
    ...
    public sealed class SkillConfigPage : ConfigPage
    {
        ...
        [SerializeField]
        private DirectHealSkillDefinitionConfig[] _directHealSkills =
    ...
    [Serializable]
    public sealed class DirectHealSkillLevelConfig
    {
        [SerializeField, Min(1)] private int _level = 1;
        ...
        public DirectHealSkillLevelConfig(
            int level,
            int healAmount,
            float range,
            float cooldown,
        ...
        internal DirectHealSkillLevelDefinition ToDomain()
        {
            return new DirectHealSkillLevelDefinition(
                _level,
                _healAmount,
                _range,
                _cooldown,
    ...
    [Serializable]
    public sealed class DirectHealSkillDefinitionConfig
    {
        [SerializeField] private string _skillId;
        [SerializeField] private string _displayName;
        [SerializeField] private SkillTargetRule _targetRule = SkillTargetRule.AllyOrSelfActor;
        [SerializeField] private DirectHealSkillLevelConfig[] _levels =
            Array.Empty<DirectHealSkillLevelConfig>();

        public DirectHealSkillDefinitionConfig(
            string skillId,
            string displayName,
            SkillTargetRule targetRule,
            DirectHealSkillLevelConfig[] levels)
        {
            _skillId = skillId;
            _displayName = displayName;
            _targetRule = targetRule;
        ...
        internal DirectHealSkillDefinition ToDomain(int index)
        {
            if (_levels == null)
            {
                throw new ArgumentException(
                    $"Direct heal skill at index {index} has no levels.");
            }

            var levels = new DirectHealSkillLevelDefinition[_levels.Length];
            for (var levelIndex = 0; levelIndex < levels.Length; levelIndex++)
            {
                levels[levelIndex] = (_levels[levelIndex] ?? throw new ArgumentException(
                    $"Direct heal skill at index {index} has a missing level at index " +
                    $"{levelIndex}.")).ToDomain();
            }

            return new DirectHealSkillDefinition(
                _skillId,
                _displayName,
                _targetRule,
                levels);
