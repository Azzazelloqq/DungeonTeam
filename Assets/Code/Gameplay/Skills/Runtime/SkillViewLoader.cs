...
namespace DungeonTeam.Gameplay.Skills.Runtime
{
    public sealed class SkillViewLoader : ISkillViewLoader
    {
            ...
            CancellationToken token)
        {
            ...
            var icons = new SkillIconViewEntry[skillIds.Count];
            var loadedAssets = new UnityEngine.Object[
                projectileSkillIds.Count + skillIds.Count * 2];
            var loadedCount = 0;
            try
            {
                for (var index = 0; index < projectileSkillIds.Count; index++)
                {
                    ...
                    var viewAssets = SkillViewAssetCatalog.Resolve(skillId);
                    var prefabObject = await _resourceLoader.LoadResourceAsync<GameObject>(
                        viewAssets.RequireProjectileAddress(skillId),
                        token);
                ...
                for (var index = 0; index < skillIds.Count; index++)
                {
                    ...
                    var address = SkillViewAssetCatalog.Resolve(skillId).PresentationAddress;
                ...
                var iconsByAddress = new Dictionary<string, Texture2D>(
                    StringComparer.Ordinal);
                for (var index = 0; index < skillIds.Count; index++)
                {
                    token.ThrowIfCancellationRequested();
                    var skillId = skillIds[index];
                    var address = SkillViewAssetCatalog.Resolve(skillId).IconAddress;
                    if (!iconsByAddress.TryGetValue(address, out var icon))
                    {
                        icon = await _resourceLoader.LoadResourceAsync<Texture2D>(
                            address,
                            token);
                        if (icon != null)
                        {
                            loadedAssets[loadedCount++] = icon;
                        }
                        ...
                        {
                            throw new InvalidOperationException(
                                $"Icon for skill ID '{skillId}' could not be loaded.");
                        }

                        iconsByAddress.Add(address, icon);
                    }

                    icons[index] = new SkillIconViewEntry(skillId, icon);
                }
